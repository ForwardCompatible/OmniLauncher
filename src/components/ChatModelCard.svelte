<script>
  ...
  let runningProc = $derived(processes.list.find((p) => p.role === "chat"));
  let availableModels = $derived(models.list.filter((m) => m.role === "chat" || m.role === null));
  ...
  async function handleLaunch() {
    ...
    try {
      await cmd.saveModelSettings(selectedModelId, form);
      loadedSnapshot = JSON.stringify(form);
      await cmd.launchModel(selectedModelId, "chat");
...
<section class="model-card">
  ...
  {#if availableModels.length === 0}
    <p class="empty-hint">No chat models found. Add a <code>.gguf</code> to the models directory.</p>
  {:else}
    <div class="field">
      <label for="model-chat">Model</label>
      <select id="model-chat" bind:value={selectedModelId} disabled={!!runningProc}>
    ...
    {#if selectedModelId !== null}
      ...
      {:else}
        ...
        <div class="field">
          <div class="label-row">
            <label for="ctx-chat">Context Size</label>
          ...
          <input id="ctx-chat" type="number" value={form.ctx_size ?? ""} placeholder="auto (0)"
